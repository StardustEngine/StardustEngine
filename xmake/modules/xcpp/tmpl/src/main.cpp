...
static int tmplRender(lua_State* lua)
{
    mustache::mustache tmpl_object { luaL_checkstring(lua, 1) };
    lua_pushstring(lua, tmpl_object.render(__internalConvertLuaValue(lua, 2)).c_str());

    return 1;
}

int luaopen(tmpl, lua_State* lua) {
